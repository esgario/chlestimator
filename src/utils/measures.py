...
class ChlMeasures:
    ...
    def set_images(self, _images, _leaf_masks=None, _bg_masks=None):
        ...
        for i in range(self.total_images):
            if _leaf_masks is not None:
                ...
                for j in range(3):
                    image[i, :, :, j] = np.ma.masked_array(
                        _images[i, :, :, j], _leaf_masks[i]
                    )
                    bg[i, :, :, j] = np.ma.masked_array(
                        _images[i, :, :, j], _bg_masks[i]
                    )
            ...
            self.H[i] = math.acos(
                (((self.R[i] - self.G[i]) + (self.R[i] - self.B[i])) / 2)
                / math.sqrt(
                    (self.R[i] - self.G[i]) ** 2
                    + (self.R[i] - self.B[i]) * (self.G[i] - self.B[i])
                )
            )
    ...
    def compute_index(self, n=None):
        switcher = {
            ...
            18: [
                lambda x, i: (x.R[i] - x.G[i]) / (x.R[i] + x.G[i] + x.B[i]),
                "R-G/R+G+B",
            ],
            19: [
                lambda x, i: (x.R[i] - x.B[i]) / (x.R[i] + x.G[i] + x.B[i]),
                "R-B/R+G+B",
            ],
            20: [
                lambda x, i: (x.G[i] - x.B[i]) / (x.R[i] + x.G[i] + x.B[i]),
                "G-B/R+G+B",
            ],
            ...
            24: [
                lambda x, i: (2 * x.g[i] - x.r[i] - x.b[i]) - (1.4 * x.r[i] - x.g[i]),
                "ExGR",
            ],
            25: [lambda x, i: (x.g[i] - x.r[i]) / (x.g[i] + x.r[i]), "NGRDI"],
            26: [
                lambda x, i: (((x.hue[i] - 60) / 60) + (1 - x.sat[i]) + (1 - x.bri[i]))
                / 3,
                "DGCI",
            ],
        }
        ...
        for i in range(self.total_images):
            ...
            sys.stdout.write(
                "\r%s - IMG[%d/%d]" % (switcher[n][1], i + 1, self.total_images)
            )
            sys.stdout.flush()
