...
def plot_multiple_indices(results, spad_measures, index_names=None, figsize=(20,20), labels=None, c_init=0, c_max=None, marker_color='k', curve_color=None):
    ...
    for i in range(rows):
        for j in range(cols):
            if c < total_subplot:
                title = index_names[c] + ' - '
                for res, spadm, mc, label in zip(results, spad_measures, marker_colors, labels):                
                    ...
                    if plot_one_curve == False:
                     ...
                     # plot scatter points
                    ax[i,j].plot(res[:,c], spadm, 'o%s' % mc, label=label, markersize=5, alpha=0.6)
